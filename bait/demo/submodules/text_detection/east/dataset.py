...
def is_cross_text(start_loc, length, vertices):
    ...
    for vertice in vertices:
        ...
        if 0.01 <= inter / (p2.area + 1e-16) <= 0.99:
            return True
    return False
...
def extract_vertices(lines):
    ...
    for line in lines:
        ...
        vertices.append(list(map(int, extract_line[:8])))
...
class custom_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512):
        ...
        assert len(self.img_files) == len(
            self.gt_files
        ), f"Different lengths: {len(self.img_files)} and {len(self.gt_files)}"
        ...
        for index in tqdm(range(len(self.img_files))):
            try:
                ...
                self.data.append((trf_img, score_map, geo_map, ignored_map))
            except:
