...
def load_pretrained_embeddings(path, unk_token=None):
    ...
    with open(path, mode="r", encoding="utf-8") as f:
        for i, line in tqdm(enumerate(f)):
            ...
            if len(vector_str) == 1:
                logging.debug(
                    f"[load_pretrained_embeddings] Ignoring row {i + 1}: {line}"
                )
                continue
...
def prepare_data(dataset, tokenizer, dataset_class, *, shuffle=False, max_length=None):
    ...
    test_encodings = tokenizer(
        test_text, truncation=True, padding=True, max_length=max_length
    )
    # encode labels
...
def prepare_text(text, tokenizer, dataset_class, *, shuffle=False, max_length=None):
    """
    Prepare data for training by creating a PyTorch dataset.
    ...
    text_encodings = tokenizer(
        text, truncation=True, padding=True, max_length=max_length
    )

    # create dataset
    text_data = dataset_class(text_encodings)
...
def load_data(key):
    ...
    if key == "MR":
        ...
        print("hallllooo")
        load_dgm_apple(dataset)
    elif key == "dgm_here":
        load_dgm_here(dataset)
...
def load_STOPS(dataset):
    ...
    :param dataset: the dataset dictionary
    """
    load_tab_spaced_data(
        "data/STOPS/STOPS/STOPS_test.txt", "data/STOPS/STOPS/STOPS_train.txt", dataset
    )
...
def load_STOPS2(dataset):
    ...
    """
    load_tab_spaced_data(
        "data/STOPS/STOPS-2/STOPS-2_test.txt",
        "data/STOPS/STOPS-2/STOPS-2_train.txt",
        dataset,
    )
...
def load_NICE(dataset):
    ...
    """
    load_tab_spaced_data(
        "data/NICE/NICE/NICE_test.txt", "data/NICE/NICE/NICE_train.txt", dataset
    )
...
def load_NICE2(dataset):
    ...
    """
    load_tab_spaced_data(
        "data/NICE/NICE_binary/NICE_test.txt",
        "data/NICE/NICE_binary/NICE_train.txt",
        dataset,
    )
...
def load_tab_spaced_data(test_path, train_path, dataset):
    ...
    label_dict = create_dict(
        set.union(
            set([data[0] for data in train_data]), set([data[0] for data in test_data])
        )
    )

    # add to dataset
    dataset["train"] = (
        [text for label, text in train_data],
        [label for label, text in train_data],
    )
    dataset["test"] = (
        [text for label, text in test_data],
        [label for label, text in test_data],
    )
...
def load_TREC(dataset):
    ...
    with open("data/TREC/TREC.txt", "r", encoding="utf-8") as f:
        data_information = [
            (int(word[0]), word[1], word[2])
            for word in [line.split() for line in f.read().splitlines()]
        ]
...
def load_Twitter(dataset):
    ...
    dataset["test"] = (
        ...
        [label for text, label in test_data],
    )
    dataset["label_dict"] = label_dict
...
def load_dgm_apple(dataset):
    """
    Load the R8 dataset.
    :param dataset: the dataset dictionary
    """
    # load training data
    with open("data/dgm_apple/train_apple.txt", "r", encoding="utf-8") as f:
        raw_train = [line.strip() for line in f]
    list_of_words = [line.split() for line in raw_train]
    # first element is the label

    train_text = [" ".join(line[1:]) for line in list_of_words]
    train_labels = [int(line[0]) for line in list_of_words]

    # load test data
    with open("data/dgm_apple/val_apple.txt", "r", encoding="utf-8") as f:
    ...
    test_labels = [int(line[0]) for line in list_of_words]
    # create label dictionary
    label_dict = create_dict(set.union(set(train_labels), set(test_labels)))
    print(label_dict)
    # add to dataset
...
def load_dgm_here(dataset):
    """
    Load the R8 dataset.
    :param dataset: the dataset dictionary
    """
    # load training data
    with open("data/dgm_here/train.txt", "r", encoding="utf-8") as f:
        raw_train = [line.strip() for line in f]
    list_of_words = [line.split() for line in raw_train]
    # first element is the label
    ...
    # load test data
    with open("data/dgm_here/test.txt", "r", encoding="utf-8") as f:
        raw_test = [line.strip() for line in f]
    list_of_words = [line.split() for line in raw_test]
    # first element is the label
    test_text = [" ".join(line[1:]) for line in list_of_words]
...
class TextPredict(torch.utils.data.Dataset):
