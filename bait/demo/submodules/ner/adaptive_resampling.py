...
class NER_Adaptive_Resampling:
    ...
    def conll_data_read(self):
        ...
        f = re.split(
            "\n\t\n|\n\n|\n \n", open(self.inputpath, "r", encoding="utf-8").read()
        )[:-1]
    ...
    def BUS(self):
        ...
        for sen in range(len(x)):
            num_sampled = len(
                [label for label in y[sen] if label not in ["B-K", "I-K", "O"]]
            )
            thres = 3 * num_sampled
            mask = [1 if label not in ["B-K", "I-K", "O"] else 0 for label in y[sen]]
...
if __name__ == "__main__":
    inputpath = "data/Data_Ner.txt"
    outputpath = "data/Data_Ner_BUS.txt"

    ner = NER_Adaptive_Resampling(inputpath, outputpath)
    # ner.resamp("nsCRD")
    ner.BUS()
