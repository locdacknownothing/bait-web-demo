...
from data import (
    ...
    build_tokenizer_for_word_embeddings,
    prepare_data_custom_tokenizer,
    prepare_data,
)
...
def train_transformer(
    ...
    if model == "ERNIE":
        ...
        test_data, train_data, label_dict = prepare_data(
            dataset, tokenizer, Dataset, max_length=512
        )
    else:

        test_data, train_data, label_dict = prepare_data(dataset, tokenizer, Dataset)

    with open("/home/dgm_bait_02/short-text-classification/dgm_label.json", "w") as f:
        json.dump(label_dict, f)

    model = AutoModelForSequenceClassification.from_pretrained(
        model_ckpt, num_labels=len(label_dict)
    ).to(device)
...
def train_mlp(
    ...
    if (pretrained_embedding_path is not None) and (pretrained_embedding_path != ""):
        ...
        vocab, embedding = load_pretrained_embeddings(
            pretrained_embedding_path, unk_token="[UNK]"
        )
        ...
        test_data, train_data, label_dict = prepare_data_custom_tokenizer(
            dataset, tokenizer, SimpleDataset
        )
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step % logging_steps == 0:
                acc, eval_loss = evaluate_model(
                    model, test_data, eval_batch_size, device, output_dir
                )
                logging.info(
                    f"Epoch {epoch} Step {step} Loss {loss.item()} Eval loss {eval_loss} Acc {acc}"
                )
...
def evaluate_model(model, test_data, eval_batch_size, device, output_dir):
    ...
    data_loader = DataLoader(
        test_data, collate_fn=collate_for_mlp, shuffle=False, batch_size=eval_batch_size
    )
    all_logits = []
    all_targets = []
...
def train_stacking(
    ...
    # setup models
    is_m1_transformer, m1, tokenizer1 = setup_model(
        device, label_dict, m1_dropout, m1_hidden_size, m1_num_layers, model1_name
    )
    is_m2_transformer, m2, tokenizer2 = setup_model(
        device, label_dict, m2_dropout, m2_hidden_size, m2_num_layers, model2_name
    )
    is_m3_transformer, m3, tokenizer3 = setup_model(
        device, label_dict, m3_dropout, m3_hidden_size, m3_num_layers, meta_model_name
    )

    model = SimplifiedStacking(
        m1,
        m2,
        m3,
        is_m1_transformer,
...
def train_weighted_boost(
    ...
    model = SimplifiedWeightedBoost(
        m1, m2, is_m1_transformer, is_m2_transformer, tokenizer1, tokenizer2
    )
    model.fit(
        dataset,
        batch_size,
...
def setup_model(device, label_dict, dropout, hidden_size, num_layers, model_name):
    ...
    # Transformer:
        ...
        m1 = AutoModelForSequenceClassification.from_pretrained(
            MODELS[model_name], num_labels=len(label_dict)
        ).to(device)
...
def main():
    ...
    # general arguments
    parser.add_argument(
        "dataset", type=str, choices=VALID_DATASETS, help="Dataset to use."
    )
    ...
    parser.add_argument(
        "--unfreeze_embedding", action="store_true", help="Unfreeze embedding."
    )

    # ensemble arguments
    parser.add_argument(
        "--m1", type=str, choices=VALID_MODELS, help="Model 1 if ensemble is used."
    )
    parser.add_argument(
        "--m2", type=str, choices=VALID_MODELS, help="Model 2 if ensemble is used."
    )
    parser.add_argument(
        "--mm",
        type=str,
        choices=VALID_MODELS,
        help="Meta model if stacking ensemble is used.",
    )

    parser.add_argument(
        "--m1_learning_rate", type=float, default=5e-5, help="Learning rate."
    )
    parser.add_argument(
        "--m1_weight_decay", type=float, default=0.00, help="Weight decay."
    )
    parser.add_argument(
        "--m1_warmup_steps", type=int, default=0, help="Number of warmup steps."
    )
    parser.add_argument("--m1_dropout", type=float, default=0.5, help="Dropout.")
    parser.add_argument(
        "--m1_num_layers", type=int, default=1, help="Number of layers."
    )
    parser.add_argument("--m1_hidden_size", type=int, default=1024, help="Hidden size.")

    parser.add_argument(
        "--m2_learning_rate", type=float, default=5e-5, help="Learning rate."
    )
    parser.add_argument(
        "--m2_weight_decay", type=float, default=0.00, help="Weight decay."
    )
    parser.add_argument(
        "--m2_warmup_steps", type=int, default=0, help="Number of warmup steps."
    )
    parser.add_argument("--m2_dropout", type=float, default=0.5, help="Dropout.")
    parser.add_argument(
        "--m2_num_layers", type=int, default=1, help="Number of layers."
    )
    ...
    parser.add_argument(
        "--mm_learning_rate", type=float, default=5e-5, help="Learning rate."
    )
    parser.add_argument(
        "--mm_weight_decay", type=float, default=0.00, help="Weight decay."
    )
    parser.add_argument(
        "--mm_warmup_steps", type=int, default=0, help="Number of warmup steps."
    )
    parser.add_argument("--mm_dropout", type=float, default=0.5, help="Dropout.")
    parser.add_argument(
        "--mm_num_layers", type=int, default=1, help="Number of layers."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight of model 1 in weighted ensemble.",
    )
    ...
    if WANDB:
        ...
        wandb.init(project="Bachelor-Thesis", name=name, config=vars(args))
        config = wandb.config
    else:
        config = vars(args)
