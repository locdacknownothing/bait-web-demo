...
class SimplifiedStacking:
    ...
    def fit(
        ...
        epochs,
        device,
        m1_num_warmup_steps=0,
        m2_num_warmup_steps=0,
        ...
        # prerequisites
        _, train_data, label_dict = prepare_data(
            dataset,
            self.tokenizer1,
        ...
        train_loader_m1 = DataLoader(
            train_data,
            collate_fn=(
                collate_for_transformer if self.is_m1_transformer else collate_for_mlp
            ),
            batch_size=batch_size,
            shuffle=False,
        )  # this has to be False, so the indexing is correct
        optimiser_m1 = AdamW(
            self.model1.parameters(), lr=m1_lr, weight_decay=m1_weight_decay
        )
        scheduler_m1 = get_linear_schedule_with_warmup(
            optimiser_m1,
            num_warmup_steps=m1_num_warmup_steps,
            num_training_steps=len(train_loader_m1) * epochs,
        )

        optimiser_m2 = AdamW(
            self.model2.parameters(), lr=m2_lr, weight_decay=m2_weight_decay
        )

        optimiser_mm = AdamW(
            self.meta_model.parameters(), lr=mm_lr, weight_decay=mm_weight_decay
        )
        ...
        for epoch in train_iterator:
            ...
            logging.debug(
                f"Correctly classified {len(correct_classified_inputs)} inputs"
            )
            ...
            if len(misclassified_inputs) > 0:
                ...
                train_encodings = self.tokenizer2(
                    train_text, truncation=True, padding=True
                )
                train_labels_encoded = [label_dict[label] for label in train_labels]
                m2_dataset = (
                    Dataset(train_encodings, train_labels_encoded)
                    if self.is_m2_transformer
                    else SimpleDataset(train_encodings, train_labels_encoded)
                )

                train_loader_m2 = DataLoader(
                    m2_dataset,
                    collate_fn=(
                        collate_for_transformer
                        if self.is_m2_transformer
                        else collate_for_mlp
                    ),
                    batch_size=batch_size,
                    shuffle=True,
                )
                scheduler_m2 = get_linear_schedule_with_warmup(
                    optimiser_m2,
                    num_warmup_steps=m2_num_warmup_steps,
            ...
            meta_dataset = (
                Dataset(X, y) if self.is_mm_transformer else SimpleDataset(X, y)
            )
            ...
            scheduler_mm = get_linear_schedule_with_warmup(
                optimiser_mm,
                num_warmup_steps=mm_num_warmup_steps,
    ...
    def evaluate(self, dataset, batch_size, device):
        ...
        test_data, _, label_dict = prepare_data(
            dataset,
            self.tokenizer3,
            Dataset if self.is_mm_transformer else SimpleDataset,
            shuffle=True,
        )
        ...
        data_loader_mm = DataLoader(
            test_data,
            collate_fn=(
                collate_for_transformer if self.is_mm_transformer else collate_for_mlp
            ),
            batch_size=batch_size,
            shuffle=False,
        )  # this has to be False, so the indexing is correct
        ...
        if len(m1_data) > 0:
            ...
            test_encodings = self.tokenizer1(
                selected_test_data, truncation=True, padding=True
            )
            test_dataset = (
                Dataset(test_encodings, selected_test_labels)
                if self.is_m1_transformer
                else SimpleDataset(test_encodings, selected_test_labels)
            )
        ...
        if len(m2_data) > 0:
            ...
            test_encodings = self.tokenizer2(
                selected_test_data, truncation=True, padding=True
            )
            test_dataset = (
                Dataset(test_encodings, selected_test_labels)
...
class SimplifiedWeightedBoost:
    ...
    def evaluate(self, dataset, batch_size, device, alpha=0.5):
        ...
        test_loader = DataLoader(
            test_data,
            collate_fn=(
                collate_for_transformer if self.is_m2_transformer else collate_for_mlp
            ),
            batch_size=batch_size,
            shuffle=False,
        )  # this has to be False, so the indexing is correct
